...
#define YELLOW_ALT \
    ESC "34m"  // based off use (identifiers in scope res) it's better to color as blue here
...
// Container utility
template<typename N>
class small_static_map
{
    const N& needle;

  public:
    small_static_map(const N& n)
        : needle(n)
    {
    }
...
#ifdef _WIN32
        DWORD windows_handle = small_static_map(fd).lookup(STDIN_FILENO,
                                                           STD_INPUT_HANDLE,
                                                           STDOUT_FILENO,
                                                           STD_OUTPUT_HANDLE,
                                                           STDERR_FILENO,
...
#define internal_verify(c, ...) \
    primitive_assert_impl(c, true, #c, LIBASSERT_PFUNC, {}, ##__VA_ARGS__)
    ...
    template<typename... T>
    LIBASSERT_ATTR_COLD std::string stringf(T... args)
    {
        int length = snprintf(nullptr, 0, args...);
        if (length < 0)
        {
            LIBASSERT_PRIMITIVE_ASSERT(false, "Invalid arguments to stringf");
        }
    ...
    template<typename C>
    LIBASSERT_ATTR_COLD static std::string join(const C& container, const std::string_view delim)
    {
    ...
    LIBASSERT_ATTR_COLD
    static void replace_all_dynamic(std::string& str,
    ...
    LIBASSERT_ATTR_COLD
    static void replace_all_template(std::string& str,
                                     const std::pair<std::regex, std::string_view>& rule)
    {
    ...
    LIBASSERT_ATTR_COLD
    static std::string indent(const std::string_view str,
                              size_t depth,
                              char c = ' ',
    ...
    template<size_t N>
    LIBASSERT_ATTR_COLD std::optional<std::array<std::string, N>> match(const std::string& s,
                                                                        const std::regex& r)
    {
        std::smatch match;
...
#    if LIBASSERT_IS_GCC
    LIBASSERT_ATTR_COLD
    static std::string resolve_addresses(const std::string& addresses,
...
#    endif
    ...
    LIBASSERT_ATTR_COLD auto get_info(ULONG type_index, HANDLE proc, ULONG64 modbase)
    {
        T info;
        if (!SymGetTypeInfo(proc,
                            modbase,
                            type_index,
    ...
    LIBASSERT_ATTR_COLD
    static std::optional<std::string> lookup_class_name(ULONG type_index,
                                                        HANDLE proc,
                                                        ULONG64 modbase)
    {
        DWORD class_parent_id =
            get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_CLASSPARENTID, true>(
                type_index, proc, modbase);
    ...
    LIBASSERT_ATTR_COLD  // returns [base, extent]
        ...
        lookup_type(ULONG type_index, HANDLE proc, ULONG64 modbase)
    {
        auto tag = get_info<SymTagEnum, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_SYMTAG>(
            type_index, proc, modbase);
        switch (tag)
        {
            ...
            case SymTagEnum::SymTagPointerType:
            {
                DWORD underlying_type_id =
                    get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_TYPEID>(
                        type_index, proc, modbase);
            ...
            case SymTagEnum::SymTagFunctionType:
            {
                ...
                n_children -= n_ignore;  // this must be ignored before
                                         // TI_FINDCHILDREN_PARAMS::Count is set, else error
                // return type
                ...
                else
                {
                    ...
                    {
                        extent += (i == 0 ? "" : ", ")
                            + resolve_type(children->ChildId[i], proc, modbase);
                    }
                    extent += ")";
            ...
            case SymTagEnum::SymTagBaseClass:
                return {get_info<WCHAR*, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_SYMNAME>(
                            type_index, proc, modbase),
                        ""};
            default:
                return {
    ...
    LIBASSERT_ATTR_COLD static std::string resolve_type(ULONG type_index,
                                                        HANDLE proc,
                                                        ULONG64 modbase)
    {
    ...
    LIBASSERT_ATTR_COLD
    static BOOL __stdcall enumerator_callback(PSYMBOL_INFO symbol_info,
    ...
    [[maybe_unused]] LIBASSERT_ATTR_COLD static std::optional<trace_t> get_stacktrace()
    {
        ...
        for (int i = 0; i < frames; i++)
        {
            ...
            if (SymFromAddr(proc, (DWORD64)addrs[i], &displacement.a, symbol))
            {
                if (got_line)
                {
                    ...
                    {
                        fprintf(stderr,
                                "Stack trace: Internal error while calling SymSetContext\n");
                        trace.push_back({line.FileName, symbol->Name, (unsigned)line.LineNumber});
                        continue;
                    }
                    DWORD n_children =
                        get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_COUNT, true>(
                            symbol->TypeIndex, proc, symbol->ModBase);
                    DWORD class_parent_id =
                        get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_CLASSPARENTID, true>(
                            symbol->TypeIndex, proc, symbol->ModBase);
                    function_info fi {proc,
                                      symbol->ModBase,
                                      0,
                                      int(n_children),
...
#    endif
    ...
    std::string prettify_type(std::string type)
    {
        ...
        // rules to replace std::basic_string -> std::string and std::basic_string_view ->
        // std::string_view
    ...
    class analysis
    {
      ...
      private:
        ...
        analysis()
        {
            ...
            std::transform(std::begin(punctuators),
                           std::end(punctuators),
                           std::begin(punctuators),
                           [&special_chars](const std::string& str) {
                               return std::regex_replace(
                                   str + (isalpha(str[0]) ? "\\b" : ""), special_chars, "\\$&");
                           });
            ...
            *std::find(std::begin(punctuators), std::end(punctuators), "<:") += "(?!:[^:>])";
            ...
            std::pair<token_e, std::string> rules_raw[] = {
                ...
                {token_e::number,
                 union_regexes(
                     {float_decimal, float_hex, int_binary, int_octal, int_hex, int_decimal})},
      ...
      public:
        ...
        std::vector<token_t> tokenize(const std::string& expression, bool decompose_shr = false)
        {
            ...
            while (i < expression.length())
            {
                ...
                {
                    if (std::regex_match(
                            std::begin(expression) + i, std::end(expression), match, re))
                    {
        ...
        LIBASSERT_ATTR_COLD
        std::vector<highlight_block> highlight(const std::string& expression)
        try
        {
            ...
            std::set<int>& output)
        {
                    ...
                    case token_e::punctuation:
                        if (operators.count(token.str))
                        {
                            ...
                            else
                            {
                                ...
                                if (token.str == "<"
                                    && find_last_non_ws(tokens, i).token_type
                                        == token_e::identifier)
                                {
                                    // branch 1: this is a template opening
                                    bool success = pseudoparse(tokens,
                                                               target_op,
                                                               i + 1,
                                ...
                                {
                                    // this must be a template parameter list, part of a generic
                                    // lambda
                                ...
                                {  // ignore precedence in template parameter list
                                    ...
                                    if (precedence.count(
                                            op))  // NOLINT(readability-braces-around-statements)
            ...
            if (middle_index != -1 && normalize_op(get_real_op(tokens, middle_index)) == target_op
                                                                 ...
                                                                 const std::string_view target_op)
        {
            ...
            for (size_t m : candidates)
            {
                ...
                fprintf(stderr,
                        "left:  %s\n",
                        libassert::detail::highlight(std::string(trim(join(left_strings, ""))))
                            .c_str());
                fprintf(stderr,
                        "right: %s\n",
                        libassert::detail::highlight(std::string(trim(join(right_strings, ""))))
                            .c_str());
            ...
            if (success && candidates.size() == 1)
            {
                ...
                // >> is decomposed and requires special handling (m will be the index of the first
                // > token)
    ...
    static std::string escape_string(const std::string_view str, char quote)
    {
        ...
        {
            if (c == '\\')
                escaped += "\\\\";  // NOLINT(readability-braces-around-statements)
            else if (c == '\t')
                escaped += "\\t";  // NOLINT(readability-braces-around-statements)
            else if (c == '\r')
                escaped += "\\r";  // NOLINT(readability-braces-around-statements)
            else if (c == '\n')
                escaped += "\\n";  // NOLINT(readability-braces-around-statements)
            else if (c == quote)
                escaped +=
                    "\\" + std::to_string(quote);  // NOLINT(readability-braces-around-statements)
            else if (c >= 32 && c <= 126)
                escaped += c;  // NOLINT(readability-braces-around-statements) // printable
            else
            {
    ...
    namespace stringification
    {
        ...
        LIBASSERT_ATTR_COLD [[nodiscard]] static std::string stringify_integral(T value,
                                                                                literal_format fmt)
        {
            ...
            switch (fmt)
            {
                ...
                default:
                    LIBASSERT_PRIMITIVE_ASSERT(false,
                                               "unexpected literal format requested for printing");
            }
            oss << value;
        r:
        ...
        LIBASSERT_ATTR_COLD [[nodiscard]] static std::string stringify_floating_point(
            T value, literal_format fmt)
        {
            std::ostringstream oss;
            switch (fmt)
            {
    ...
    struct column_t
    {
        ...
        LIBASSERT_ATTR_COLD column_t(size_t _width,
                                     std::vector<highlight_block> _blocks,
                                     bool _right_align = false)
            : width(_width)
    ...
    LIBASSERT_ATTR_COLD [[nodiscard]] std::string print_stacktrace(trace_t* raw_trace,
                                                                   int term_width)
    {
        ...
        if (raw_trace)
        {
            ...
            const auto max_line_number =
                std::max_element(trace.begin() + start,
                                 trace.begin() + end + 1,
            ...
            for (size_t i = start; i <= end; i++)
            {
                ...
                if (term_width >= 50)
                {
                    ...
                    size_t middle =
                        std::max(line_number.size(),
                                 max_line_number_width);  // todo: is this looking right...?
    ...
    binary_diagnostics_descriptor::binary_diagnostics_descriptor(
        binary_diagnostics_descriptor&&) noexcept = default;
    LIBASSERT_ATTR_COLD binary_diagnostics_descriptor& binary_diagnostics_descriptor::operator=(
        binary_diagnostics_descriptor&&) noexcept = default;
    ...
    LIBASSERT_ATTR_COLD [[nodiscard]] std::string print_binary_diagnostics(
        size_t term_width, binary_diagnostics_descriptor& diagnostics)
    {
        ...
        // determine whether we actually gain anything from printing a where clause (e.g. exclude "1
        // => 1")
        struct
        {
            bool left, right;
        } has_useful_where_clause = {
            multiple_formats || lstrings.size() > 1
                || (a_str != lstrings[0] && trim_suffix(a_str) != lstrings[0]),
            multiple_formats || rstrings.size() > 1
        ...
        if (has_useful_where_clause.left || has_useful_where_clause.right)
        {
            ...
            where += "    Where:\n";
            auto print_clause =
                [term_width, lw, &where](const char* expr_str, std::vector<std::string>& expr_strs)
            {
                if (term_width >= min_term_width)
                {
                    where +=
                        wrapped_print({{where_indent - 1,
                                        {{"", ""}}},  // 8 space indent, wrapper will add a space
                                       {lw, highlight_blocks(expr_str)},
                                       {2, {{"", "=>"}}},
                                       {term_width - lw - 8 /* indent */ - 4 /* arrow */,
                                        get_values(expr_strs)}});
                }
                else
                {
                    where += stringf("        %s%*s => ",
    ...
    LIBASSERT_ATTR_COLD [[nodiscard]] std::string print_extra_diagnostics(
        size_t term_width, const decltype(extra_diagnostics::entries)& extra_diagnostics)
    {
    ...
    LIBASSERT_ATTR_COLD extra_diagnostics::~extra_diagnostics() = default;
    LIBASSERT_ATTR_COLD extra_diagnostics::extra_diagnostics(extra_diagnostics&&) noexcept =
        default;
...
namespace libassert
{
    ...
    LIBASSERT_ATTR_COLD std::string assertion_printer::operator()(int width) const
    {
        ...
        output +=
            stringf("    %s\n",
                    highlight(stringf("%s(%s%s);", name, expr_str, sizeof_args > 0 ? ", ..." : ""))
                        .c_str());
    ...
    LIBASSERT_ATTR_COLD
    std::tuple<const char*, int, std::string, const char*> assertion_printer::get_assertion_info()
        const
    {
...
LIBASSERT_ATTR_COLD
void libassert_default_fail_action(libassert::assert_type type,
                                   ASSERTION fatal,
                                   const libassert::assertion_printer& printer)
{
    libassert::detail::enable_virtual_terminal_processing_if_needed();  // for terminal colors on
                                                                        // windows
    ...
    switch (type)
    {
        ...
        case libassert::assert_type::assertion:
            ...
            // debug build of this library (on top of preventing fallthrough from nonfatal
            // libassert)
            break;
