...
#        pragma warning(push)
#        pragma warning( \
                disable : 4251)  // class 'xxx' needs to have dll-interface to be used by clients of
                                 // struct 'xxx' // when IMGUI_API is set to__declspec(dllexport)
#        pragma warning(disable : 26812)  // The enum type 'xxx' is unscoped. Prefer 'enum class'
                                          // over 'enum' (Enum.3). [MSVC Static Analyzer)
#        pragma warning(disable : 26495)  // [Static Analyzer] Variable 'XXX' is uninitialized.
                                          // Always initialize a member variable (type.6).
#        if defined(_MSC_VER) && _MSC_VER >= 1922  // MSVC 2019 16.2 or later
#            pragma warning(disable : 5054)  // operator '|': deprecated between enumerations of
                                             // different types
#        endif
#    endif
...
#        pragma clang diagnostic ignored \
            "-Wimplicit-int-float-conversion"  // warning: implicit conversion from 'xxx' to 'float'
                                               // may lose precision
#        pragma clang diagnostic ignored \
            "-Wmissing-noreturn"  // warning: function 'xxx' could be declared with attribute
                                  // 'noreturn'
...
#        pragma GCC diagnostic ignored \
            "-Wclass-memaccess"  // [__GNUC__ >= 8] warning: 'memset/memcpy' clearing/writing an
...
struct ImGuiDockNodeSettings;  // Storage for a dock node in .ini file (we preserve those even if
                               // the associated dock node isn't active during the session)
...
#    ifdef _WIN32
#        define IM_NEWLINE \
            "\r\n"  // Play it nice with Windows users (Update: since 2018-05, Notepad finally
                    // appears to support Unix-style carriage returns!)
#    else
#        define IM_NEWLINE "\n"
#    endif
#    ifndef IM_TABSIZE  // Until we move this to runtime and/or add proper tab support, at least
                        // allow users to compile-time override
#        define IM_TABSIZE (4)
#    endif
...
#            define IM_DEBUG_BREAK() \
                IM_ASSERT(0)  // It is expected that you define IM_DEBUG_BREAK() into something that
                              // will break nicely in a debugger!
#        endif
...
#    else
#        define IMGUI_DISABLE_TTY_FUNCTIONS  // Can't use stdout, fflush if we are not using default
                                             // file functions
#    endif
...
#    define IM_BITARRAY_TESTBIT(_ARRAY, _N) \
        ( \
            ...
            != 0)  // Macro version of ImBitArrayTestBit(): ensure args have side-effect or are
                   // costly!
...
#    define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD, _MAXERROR) \
        ImClamp(IM_ROUNDUP_TO_EVEN( \
                    (int)ImCeil(IM_PI / ImAcos(1 - ImMin((_MAXERROR), (_RAD)) / (_RAD)))), \
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_ButtonRepeat = 1
        << 1,  // false     // Button() will return true multiple times based on io.KeyRepeatDelay
    ...
    ImGuiItemFlags_NoNav = 1 << 3,  // false     // Disable keyboard/gamepad directional navigation
    ...
    ImGuiItemFlags_NoWindowHoverableCheck = 1
        << 8,  // false     // Disable hoverable check in ItemHoverable()
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HoveredRect = 1
        << 0,  // Mouse position is within item rectangle (does NOT mean that the window is in
    ...
    ImGuiItemStatusFlags_ToggledSelection = 1
        << 3,  // Set when Selectable(), TreeNode() reports toggling a selection. We can't report
               ...
               // issues.
    ImGuiItemStatusFlags_ToggledOpen = 1
        << 4,  // Set when TreeNode() reports toggling their open state.
    ImGuiItemStatusFlags_HasDeactivated = 1
        << 5,  // Set if the widget/group is able to provide data for the
               // ImGuiItemStatusFlags_Deactivated flag.
    ImGuiItemStatusFlags_Deactivated = 1
        << 6,  // Only valid if ImGuiItemStatusFlags_HasDeactivated is set.
    ...
    ImGuiItemStatusFlags_Visible = 1
        << 9,  // [WIP] Set when item is overlapping the current clipping rectangle (Used
               // internally. Please don't use yet: API/system will change as we refactor
               // Itemadd()).
...
enum ImGuiInputTextFlagsPrivate_
{
    ...
    ImGuiInputTextFlags_NoMarkEdited = 1
        << 27,  // For internal use by functions using InputText() before reformatting data
    ImGuiInputTextFlags_MergedItem = 1
        << 28,  // For internal use by TempInputText(), will skip calling ItemAdd(). Require
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_PressedOnClickRelease = 1
        << 5,  // [Default] return true on click + release on same item <-- this is what the
               // majority of Button are using
    ImGuiButtonFlags_PressedOnClickReleaseAnywhere = 1
        << 6,  // return true on click + release even if the release event is not done while
               // hovering the item
    ImGuiButtonFlags_PressedOnRelease = 1
        << 7,  // return true on release (default requires click+release)
    ImGuiButtonFlags_PressedOnDoubleClick = 1
        << 8,  // return true on double-click (default requires click+release)
    ImGuiButtonFlags_PressedOnDragDropHold = 1
        << 9,  // return true when held into while we are drag and dropping another item (used by
               // e.g. tree nodes, collapsing headers)
    ImGuiButtonFlags_Repeat = 1 << 10,  // hold to repeat
    ImGuiButtonFlags_FlattenChildren = 1
        << 11,  // allow interactions even if a child window is overlapping
    ImGuiButtonFlags_AllowItemOverlap = 1
        << 12,  // require previous frame HoveredId to either match id or be null before being
                // usable, use along with SetItemAllowOverlap()
    ImGuiButtonFlags_DontClosePopups = 1
    ...
    ImGuiButtonFlags_AlignTextBaseLine = 1
        << 15,  // vertically align button to match text baseline - ButtonEx() only // FIXME: Should
    ...
    ImGuiButtonFlags_NoHoldingActiveId = 1
        << 17,  // don't set ActiveId while holding the mouse (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_NoNavFocus = 1
        << 18,  // don't override navigation focus when activated (FIXME: this is essentially used
                ...
                // g.LastItemData.InFlags)
    ImGuiButtonFlags_NoHoveredOnFocus = 1
        << 19,  // don't report as hovered when nav focus is on this item
    ImGuiButtonFlags_NoSetKeyOwner = 1
        << 20,  // don't set key/input owner on the initial click (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
    ImGuiButtonFlags_NoTestKeyOwner = 1
        << 21,  // don't test key/input owner when polling the key (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
    ImGuiButtonFlags_PressedOnMask_ = ImGuiButtonFlags_PressedOnClick
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnNav = 1
        << 21,  // (WIP) Auto-select when moved into. This is not exposed in public API as to handle
                // multi-select and modifiers we will need user to explicitly control focus scope.
                // May be replaced with a BeginSelection() API.
    ImGuiSelectableFlags_SelectOnClick = 1
        << 22,  // Override button behavior to react on Click (default is Click+Release)
    ImGuiSelectableFlags_SelectOnRelease = 1
        << 23,  // Override button behavior to react on Release (default is Click+Release)
    ImGuiSelectableFlags_SpanAvailWidth = 1
        << 24,  // Span all avail width even if we declared less for layout purpose. FIXME: We may
                // be able to remove this (added in 6251d379, 2bcafc86 for menus)
    ImGuiSelectableFlags_SetNavIdOnHover = 1
        << 25,  // Set Nav/Focus ID on mouse hover (used by MenuItem)
    ImGuiSelectableFlags_NoPadWithHalfSpacing = 1
        << 26,  // Disable padding each side with ItemSpacing * 0.5f
    ImGuiSelectableFlags_NoSetKeyOwner = 1
        << 27,  // Don't set key/input owner on the initial click (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
};
...
enum ImGuiTooltipFlags_
{
    ...
    ImGuiTooltipFlags_OverridePreviousTooltip = 1
        << 0,  // Override will clear/ignore previously submitted tooltip (defaults to append)
};
...
#    define ImGuiKeyOwner_Any \
        ((ImGuiID)0)  // Accept key that have an owner, UNLESS a call to SetKeyOwner() explicitly
                      // used ImGuiInputFlags_LockThisFrame or ImGuiInputFlags_LockUntilRelease.
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_LockThisFrame = 1
        << 6,  // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT
    ...
    ImGuiInputFlags_LockUntilRelease = 1
        << 7,  // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT
               // accepted for polling). Cleared when the key is released or at end of each frame if
               // key is released. This is useful to make input-owner-aware code steal keys from
    ...
    ImGuiInputFlags_RouteFocused = 1
        << 8,  // (Default) Register focused route: Accept inputs if window is in focus stack.
               // Deep-most focused window takes inputs. ActiveId takes inputs over deep-most
               // focused window.
    ImGuiInputFlags_RouteGlobalLow = 1
        << 9,  // Register route globally (lowest priority: unless a focused window or active item
               // registered the route) -> recommended Global priority.
    ImGuiInputFlags_RouteGlobal = 1
        << 10,  // Register route globally (medium priority: unless an active item registered the
                // route, e.g. CTRL+A registered by InputText).
    ImGuiInputFlags_RouteGlobalHigh = 1
        << 11,  // Register route globally (highest priority: unlikely you need to use that: will
    ...
    ImGuiInputFlags_RouteUnlessBgFocused = 1
        << 13,  // Global routes will not be applied if underlying background/void is focused (== no
...
enum ImGuiActivateFlags_
{
    ...
    ImGuiActivateFlags_PreferInput = 1
        << 0,  // Favor activation that requires keyboard text input (e.g. for Slider/Drag). Default
               // if keyboard is available.
    ImGuiActivateFlags_PreferTweak = 1
        << 1,  // Favor activation for tweaking with arrows or gamepad (e.g. for Slider/Drag).
               // Default if keyboard is not available.
    ImGuiActivateFlags_TryToPreserveState = 1
        << 2,  // Request widget to preserve state if it can (e.g. InputText will try to preserve
               // cursor/selection)
};
...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX = 1
        << 0,  // If item is not visible: scroll as little as possible on X axis to bring item back
               // into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY = 1
        << 1,  // If item is not visible: scroll as little as possible on Y axis to bring item back
               // into view [default for Y axis for windows that are already visible]
    ImGuiScrollFlags_KeepVisibleCenterX = 1
        << 2,  // If item is not visible: scroll to make the item centered on X axis [rarely used]
    ImGuiScrollFlags_KeepVisibleCenterY = 1
        << 3,  // If item is not visible: scroll to make the item centered on Y axis
    ImGuiScrollFlags_AlwaysCenterX = 1
        << 4,  // Always center the result item on X axis [rarely used]
    ImGuiScrollFlags_AlwaysCenterY = 1
        << 5,  // Always center the result item on Y axis [default for Y axis for appearing window)
    ImGuiScrollFlags_NoScrollParent = 1
        << 6,  // Disable forwarding scrolling to parent window if required to keep item/rect
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId = 1
        << 4,  // Allow scoring and considering the current NavId as a move target candidate. This
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet = 1
        << 5,  // Store alternate result in NavMoveResultLocalVisible that only comprise elements
               // that are already fully visible (used by PageUp/PageDown)
    ImGuiNavMoveFlags_ScrollToEdgeY = 1
        << 6,  // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim to remove or
...
enum ImGuiOldColumnFlags_
{
    ...
    ImGuiOldColumnFlags_NoPreserveWidths = 1
    ...
    ImGuiOldColumnFlags_GrowParentContentsSize = 1
        << 4,  // (WIP) Restore pre-1.51 behavior of extending the parent window contents size but
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_DockSpace = 1
        << 10,  // Local, Saved  // A dockspace is a node that occupy space within an existing user
                // window. Otherwise the node is floating and create its own window.
    ImGuiDockNodeFlags_CentralNode = 1
        << 11,  // Local, Saved  // The central node has 2 main properties: stay visible when empty,
                // only use "remaining" spaces from its neighbor.
    ImGuiDockNodeFlags_NoTabBar = 1 << 12,  // Local, Saved  // Tab bar is completely unavailable.
                                            // No triangle in the corner to enable it back.
    ImGuiDockNodeFlags_HiddenTabBar = 1
        << 13,  // Local, Saved  // Tab bar is hidden, with a triangle in the corner to show it
                // again (NB: actual tab-bar instance may be destroyed as this is only used for
                // single-window tab bar)
    ImGuiDockNodeFlags_NoWindowMenuButton = 1
        << 14,  // Local, Saved  // Disable window/docking menu (that one that appears instead of
                // the collapse button)
    ImGuiDockNodeFlags_NoCloseButton = 1 << 15,  // Local, Saved  //
    ImGuiDockNodeFlags_NoDocking = 1
        << 16,  // Local, Saved  // Disable any form of docking in this dockspace or individual
                // node. (On a whole dockspace, this pretty much defeat the purpose of using a
                // dockspace at all). Note: when turned on, existing docked nodes will be preserved.
    ImGuiDockNodeFlags_NoDockingSplitMe = 1
        << 17,  // [EXPERIMENTAL] Prevent another window/node from splitting this node.
    ImGuiDockNodeFlags_NoDockingSplitOther = 1
        << 18,  // [EXPERIMENTAL] Prevent this node from splitting another window/node.
    ImGuiDockNodeFlags_NoDockingOverMe = 1
        << 19,  // [EXPERIMENTAL] Prevent another window/node to be docked over this node.
    ImGuiDockNodeFlags_NoDockingOverOther = 1
        << 20,  // [EXPERIMENTAL] Prevent this node to be docked over another window or non-empty
                // node.
    ImGuiDockNodeFlags_NoDockingOverEmpty = 1
        << 21,  // [EXPERIMENTAL] Prevent this node to be docked over an empty node (e.g. DockSpace
...
struct IMGUI_API ImGuiDockNode
{
    ...
    bool WantMouseMove : 1;  // After a node extraction we need to transition toward moving the
                             // newly created host window
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    int LastFrontMostStampCount;  // Last stamp number from when a window hosted by this viewport
                                  // was made front-most (by comparing this value between two
                                  // viewport we have an implicit viewport z-order
    ImGuiID LastNameHash;
    ImVec2 LastPos;
    float Alpha;  // Window opacity (when dragging dockable windows/viewports we make them
                  // transparent)
    float LastAlpha;
    ...
    {
        Idx = -1;
        LastFrameActive = DrawListsLastFrame[0] = DrawListsLastFrame[1] = LastFrontMostStampCount =
            -1;
        LastNameHash = 0;
...
{
    ImGuiID ID;
    ImVec2ih Pos;  // NB: Settings position are stored RELATIVE to the viewport! Whereas runtime
                   // ones are absolute positions.
    ImVec2ih Size;
...
#    endif
    ...
    ImGuiViewportP* MouseLastHoveredViewport;  // Last known viewport that was hovered by mouse
    ...
    int ViewportFrontMostStampCount;  // Every time the front-most window changes, we stamp its
    ...
    ImGuiID NavActivateId;  // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) ||
                            // IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also set when
                            // calling ActivateItem()
    ImGuiID NavActivateDownId;  // ~~ IsKeyDown(ImGuiKey_Space) ||
                                // IsKeyDown(ImGuiKey_NavGamepadActivate) ? NavId : 0
    ImGuiID
        NavActivatePressedId;  // ~~ IsKeyPressed(ImGuiKey_Space) ||
                               // IsKeyPressed(ImGuiKey_NavGamepadActivate) ? NavId : 0 (no repeat)
    ImGuiID NavActivateInputId;  // ~~ IsKeyPressed(ImGuiKey_Enter) ||
                                 // IsKeyPressed(ImGuiKey_NavGamepadInput) ? NavId : 0;
                                 // ImGuiActivateFlags_PreferInput will be set and NavActivateId
                                 // will be 0.
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_SaveSettings = 1
        << 22,  // FIXME: Settings are handled by the docking system, this only request the tab bar
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton = 1
        << 20,  // Track whether p_open was set or not (we'll need this info on the next frame to
    ...
    ImGuiTabItemFlags_Unsorted = 1 << 22,  // [Docking] Trailing tabs with the _Unsorted flag will
                                           // be sorted based on the DockOrder of their Window.
...
namespace ImGui
{
    ...
    IMGUI_API void
    Shutdown();  // Since 1.60 this is a _private_ function. You can call DestroyContext() to
                 // destroy the context created by CreateContext().
    ...
    IMGUI_API bool
    IsItemToggledSelection();  // Was the last item selection toggled? (after Selectable(),
                               // TreeNode() etc. We only returns toggle _event_ in order to handle
                               // clipping correctly)
    ...
    IMGUI_API void LogBegin(
        ImGuiLogType type, int auto_open_depth);  // -> BeginCapture() when we design v2 api, for
    ...
    IMGUI_API void ActivateItem(
        ImGuiID id);  // Remotely activate a button, checkbox, tree node etc. given its unique ID.
    ...
    IMGUI_API bool IsKeyPressed(
        ...
        ImGuiID owner_id,
        ImGuiInputFlags flags =
            0);  // Important: when transitioning from old to new IsKeyPressed(): old API has "bool
                 // repeat = true", so would default to repeat. New API requiress explicit
                 // ImGuiInputFlags_Repeat.
    ...
    IMGUI_API void DockContextProcessUndockWindow(ImGuiContext* ctx,
                                                  ImGuiWindow* window,
    ...
    IMGUI_API void BeginColumns(
        ...
        int count,
        ImGuiOldColumnFlags flags =
            0);  // setup number of columns. use an identifier to distinguish multiple column sets.
    ...
    TableGetHoveredColumn();  // May use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered)
                              // instead. Return hovered column. return -1 when table is not
                              // hovered. return columns_count if the unused space at the right of
    ...
    IMGUI_API void RenderTextEllipsis(ImDrawList* draw_list,
                                      ...
                                      const char* text_end,
                                      const ImVec2* text_size_if_known);
    IMGUI_API void RenderFrame(
        ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border = true, float rounding = 0.0f);
    ...
    IMGUI_API void RenderArrow(
        ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale = 1.0f);
    ...
    IMGUI_API bool TreeNodeUpdateNextOpen(
        ImGuiID id,
        ImGuiTreeNodeFlags flags);  // Return open state. Consume previous SetNextItemOpen() data,
    ...
    IMGUI_API void DebugLocateItemOnHover(
        ImGuiID
            target_id);  // Only call on reaction to a mouse Hover: because only 1 at the same time!
    ...
    inline void DebugDrawItemRect(ImU32 col = IM_COL32(255, 0, 0, 255))
    {
        ...
        GetForegroundDrawList(window)->AddRect(
            g.LastItemData.Rect.Min, g.LastItemData.Rect.Max, col);
    }
