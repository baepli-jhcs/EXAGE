...
//  2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not being honored +
//  removed a bunch of duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our
//  switch to C++11. 2022-10-04: Vulkan: Added experimental ImGui_ImplVulkan_RemoveTexture() for api
//  symetry. (#914, #5738). 2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet.
//  User need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define
//  ImTextureID ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a
//  full-viewport to reduce likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their
//  app without calling vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized backend to
//  pull data from a single structure to facilitate usage with multiple-contexts (all g_XXXX access
//  changed to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes
//  are not multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change
//  blending equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added support for
//  custom function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using
//  ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for specifying which subpass
//  to reference during VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline parameter to
//  ImGui_ImplVulkan_RenderDrawData (default to one passed to ImGui_ImplVulkan_Init). 2020-05-04:
//  Vulkan: Fixed crash if initial frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where
//  render callbacks wouldn't be called if the ImDrawData didn't have vertices. 2019-08-01: Vulkan:
//  Added support for specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to
//  one of the VkSampleCountFlagBits values to use, default is non-multisampled as before.
...
//  ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04: *BREAKING CHANGE*:
//  Vulkan: Added ImageCount/MinImageCount fields in ImGui_ImplVulkan_InitInfo, required for
//  initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added
//  ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added VkInstance argument to
//  ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04: Vulkan: Avoid passing negative
...
//  2018-06-22: Inverted the parameters to ImGui_ImplVulkan_RenderDrawData() to be consistent with
//  other backends. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined
//  GLFW+Vulkan example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to
//  setup projection matrix and clipping rectangle. 2018-03-03: Vulkan: Various refactor, created a
//  couple of ImGui_ImplVulkanH_XXX helper that the example can use and that viewport support will
//  use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and
//  fields to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the
//  io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData()
//  itself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60
//  WIP, user needs to call CreateContext/DestroyContext themselves. 2017-05-15: Vulkan: Fix scissor
//  offset being negative. Fix new Vulkan validation warnings. Set required depth member for buffer
//  image copy. 2016-11-13: Vulkan: Fix validation layer warnings and errors and redeclare
//  gl_PerVertex. 2016-10-18: Vulkan: Add location decorators & change to use structs as in/out in
                                     ...
                                     VkPipeline pipeline)
{
    ...
    if (wrb->FrameRenderBuffers == nullptr)
    {
        ...
        memset(
            wrb->FrameRenderBuffers, 0, sizeof(ImGui_ImplVulkanH_FrameRenderBuffers) * wrb->Count);
    }
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        check_vk_result(err);
        err = vkMapMemory(
            v->Device, rb->IndexBufferMemory, 0, rb->IndexBufferSize, 0, (void**)(&idx_dst));
        check_vk_result(err);
        for (int n = 0; n < draw_data->CmdListsCount; n++)
        {
            const ImDrawList* cmd_list = draw_data->CmdLists[n];
            memcpy(
                vtx_dst, cmd_list->VtxBuffer.Data, cmd_list->VtxBuffer.Size * sizeof(ImDrawVert));
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ImGui_ImplVulkan_SetupRenderState(
                        draw_data, pipeline, command_buffer, rb, fb_width, fb_height);
                else
            ...
            else
            {
                ...
                if (pcmd->TextureId == static_cast<const ImTextureID>(bd->FontImage))
                {
                    descriptor.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                    descriptor.imageView = bd->FontView;
                    descriptor.sampler = bd->FontSampler;
                }
                else
                {
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    // Copy to Image:
    {
        ...
        vkCmdPipelineBarrier(bd->FontCommandBuffer,
        ...
        vkCmdCopyBufferToImage(bd->FontCommandBuffer,
                               upload_buffer,
                               bd->FontImage,
...
#undef IMGUI_VULKAN_FUNC_MAP_MACRO
    ...
    IM_ASSERT(info->ColorAttachmentFormat != VK_FORMAT_UNDEFINED);
...
void ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(VkDevice device,
                                                        const VkAllocationCallbacks* allocator)
{
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    VkResult err = (VkResult)platform_io.Platform_CreateVkSurface(
        viewport, (ImU64)v->Instance, (const void*)v->Allocator, (ImU64*)&wd->Surface);
    check_vk_result(err);
    ...
    wd->SurfaceFormat =
        ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice,
                                              wd->Surface,
    ...
    // Select Present Mode
    // FIXME-VULKAN: Even thought mailbox seems to get us maximum framerate with a single window, it
    // halves framerate with a second window etc. (w/ Nvidia and SDK 1.82.1)
    VkPresentModeKHR present_modes[] = {
        VK_PRESENT_MODE_MAILBOX_KHR, VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_FIFO_KHR};
    wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
        v->PhysicalDevice, wd->Surface, &present_modes[0], IM_ARRAYSIZE(present_modes));
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    if (vd == nullptr)  // This is nullptr for the main viewport (which is left to the user/app to
                        // handle)
        return;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    {
        {
            err = vkAcquireNextImageKHR(v->Device,
                                        wd->Swapchain,
                                        UINT64_MAX,
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    wd->FrameIndex =
        (wd->FrameIndex + 1) % wd->ImageCount;  // This is for the next vkWaitForFences()
    wd->SemaphoreIndex =
